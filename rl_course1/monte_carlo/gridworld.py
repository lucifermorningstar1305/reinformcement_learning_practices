...
class GridWorld(object):
    ...
    def reset(self):
        """Function to reset the agent to the initial starting state."""
        self.pos_i = 2
        self.pos_j = 0
    ...
    def game_over(self):
        ...
        return (self.pos_i, self.pos_j) not in self.actions
...
class WindyGridWorld(object):
    ...
    def get_all_states(self):
        """Function to return the complete space"""
        all_states = set(self.actions.keys()) | set(self.rewards.keys())
        return all_states
    ...
    def is_terminal(self, state: Tuple):
        """Function to check if the current state is a terminal state or not."""
        return state not in self.actions
...
def negative_grid(step_cost: float = -0.1):
    """Function to setup a standard grid with step_loss"""
...
def windy_grid_penalized(step_cost: float = -0.1) -> WindyGridWorld:
    """Function to setup a standard WindyGridWorld environment but with penalty."""
